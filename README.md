
<h1 align="center">✨ Welcome to Open-Source ✨</h1>
<h2 align ="center">version1.1🥇</h2>
<div align="center">
  <p>
    <br>
    I'm <span style="color:#00bfff;">project_M</span>, DIVYANSHU😁 ARJUN KESHAV😎.<br>
   TEAM OF 3 MEMBERS 🚀
  </p>
</div>

<hr style="border: 1px solid #00bfff; width: 80%;">

<div style="display:flex; justify-content:center; gap:20px; flex-wrap:wrap; text-align:center;">
  <div style="background:#1e1e1e; color:#fff; padding:15px 25px; border-radius:12px; width:220px; box-shadow:0 4px 8px rgba(0,0,0,0.3);">
    <h3 style="color:#00bfff;">💻 Skills</h3>
    <p>Python, C++, HTML, CSS, Arduino</p>
  </div>

  <div style="background:#1e1e1e; color:#fff; padding:15px 25px; border-radius:12px; width:220px; box-shadow:0 4px 8px rgba(0,0,0,0.3);">
    <h3 style="color:#00ff99;">🚀 Projects</h3>
    <p>PROJECT_M💦💦</p>
  </div>

   <div class="dropdown">
    <button class="dropbtn">🌟Team Members</button>
    <div class="dropdown-content">
      <H2>⚙️DIVYANSHU<H2>
      <H2>💻ARJUN<H2>
      <H2>👨‍💻KESHAV SINGH</H2>
      
   
  </div>
</div>

<hr style="border: 1px solid #00bfff; width: 80%; margin-top:40px;">

<p align="center">
  <i>"Code. Create. Conquer." 🧠</i>
</p>
